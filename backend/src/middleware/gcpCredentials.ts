...
export function gcpCredentialsMiddleware(
  ...
  // Service account credentials can be passed as base64-encoded JSON
  // via the x-gcp-credentials header, or fall back to env vars.
  ...
  if (!clientEmail) {
    clientEmail = normalize(process.env.GCP_CLIENT_EMAIL);
  }
  if (!privateKey) {
    privateKey = normalize(process.env.GCP_PRIVATE_KEY)?.replace(/\\n/g, '\n');
  }
