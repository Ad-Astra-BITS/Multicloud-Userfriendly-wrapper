 ...
 * ConnectGCPModal.tsx
 *
 * Modal for connecting a Google Cloud Platform project to Ad Astra via a
 * Service Account Key (JSON). Mirrors ConnectDOModal.tsx in structure.
 *
 * Flow:
 *   1. User enters their GCP Project ID and pastes a service account key JSON.
 *   2. On submit, POST /api/gcp/validate is called.
 *   3. The backend attempts a Compute Engine list to verify the credentials.
 *   4. On success, credentials are saved to sessionStorage via GCPContext.
 */
...
export default function ConnectGCPModal() {
  ...
  function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      // Try to auto-fill project ID from the key file
      try {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!projectId.trim()) {
      setError('A GCP Project ID is required.');
      return;
    }

    setLoading(true);
    try {
      // Encode service account JSON as base64 for header transport
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
            ...
            <div className="text-sm text-slate-300">
              ...
              Your service account key is stored only in{' '}
              <code className="text-blue-300 bg-blue-500/10 px-1 rounded">sessionStorage</code>{' '}
              and sent as a header on API calls. It is never logged or saved to our database.
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                ...
                <a
                  ...
                  className="text-red-400 hover:text-red-300 inline-flex items-center gap-0.5"
                >
            ...
            <div>
              ...
              <div className="space-y-2">
                {/* File upload */}
                <label className="flex items-center gap-2 px-3 py-2 bg-slate-800 border border-slate-600 rounded-lg text-sm text-slate-400 hover:border-slate-500 cursor-pointer transition-colors">
                  <FileJson size={14} />
                  <span>{serviceAccountJson ? 'Key file loaded ✓' : 'Upload key file (.json)'}</span>
                  <input
                    type="file"
                    accept=".json"
                ...
                <div className="relative">
                  <textarea
                    value={showJson ? serviceAccountJson : serviceAccountJson ? '••• JSON key loaded •••' : ''}
                    onChange={(e) => { setShowJson(true); setServiceAccountJson(e.target.value); }}
                    onFocus={() => setShowJson(true)}
                    placeholder='Or paste the JSON key here...'
                    rows={4}
                    disabled={loading || !!success}
              ...
              <p className="text-xs text-slate-500 mt-1">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-yellow-400 hover:text-yellow-300 inline-flex items-center gap-0.5"
                >
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Optional when using Application Default Credentials (ADC) on the server.
              </p>
            </div>
              ...
              <div className="flex items-start gap-3 p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
                ...
                <div className="text-sm text-green-300">
                  ...
                  <p className="text-green-400 mt-0.5">
                    Project: <span className="font-mono">{success.projectId}</span>
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                type="submit"
                disabled={loading || !!success || !projectId.trim()}
                className="flex-1 py-2.5 text-white bg-red-600 hover:bg-red-500 disabled:bg-slate-700 disabled:text-slate-400 rounded-xl font-medium text-sm transition-colors flex items-center justify-center gap-2"
              >
                {loading ? (
          ...
          {/* Required IAM roles hint */}
          <div className="p-4 bg-slate-800/60 border border-slate-700/40 rounded-xl text-xs text-slate-500">
            <p className="font-medium text-slate-400 mb-1">Required IAM roles for the service account:</p>
